...
export class CubicNoiseTexture {
    ...
    public setData(v: number, i: number, j: number, k: number): void {
        while (i < 0) {
            i += this.size;
        }
        while (j < 0) {
    ...
    public get3DTexture(): RawTexture3D{
        ...
        console.log(min + " " + max);
        let tex = new RawTexture3D(data, this.size, this.size, this.size, Constants.TEXTUREFORMAT_R, this.scene, false, false, Texture.TRILINEAR_SAMPLINGMODE, Engine.TEXTURETYPE_UNSIGNED_BYTE);
        tex.wrapU = 1;
        tex.wrapV = 1;
        tex.wrapR = 1;
