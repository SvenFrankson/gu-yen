...
export class ExtendedVertexData {
    ...
    constructor(public vertexData: VertexData) {
        if (vertexData.positions && vertexData.indices) {
            for (let i = 0; i < vertexData.indices.length / 3; i++) {
                ...
                this.triangles.push([new Vector3(Math.floor(x1 * 2), Math.floor(y1 * 2), Math.floor(z1 * 2)), new Vector3(Math.floor(x2 * 2), Math.floor(y2 * 2), Math.floor(z2 * 2)), new Vector3(Math.floor(x3 * 2), Math.floor(y3 * 2), Math.floor(z3 * 2))]);
                ...
                this.fastColorIndex.push([new Vector3(Math.round(color1.x), Math.round(color1.y), Math.round(color1.z)), new Vector3(Math.round(color2.x), Math.round(color2.y), Math.round(color2.z)), new Vector3(Math.round(color3.x), Math.round(color3.y), Math.round(color3.z))]);
            }
        }
    }
}
