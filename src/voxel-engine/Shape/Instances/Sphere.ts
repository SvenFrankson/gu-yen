...
export class Sphere extends Shape {
    ...
    constructor(terrain: Terrain, public props?: ISphereProps) {
        super(terrain);
        if (!this.props) {
            this.props = {};
        }
        if (!IsVeryFinite(this.props.diameter)) {
            this.props.diameter = 2;
        }
    }
    ...
    public draw(chunck: Chunck, ijk: IJK, dir: number, blockType: BlockType, mode: TerrainEditionMode, saveToLocalStorage?: boolean) {
        ...
        let first = - Math.floor(this.props.diameter / 2);
        let last = Math.ceil(this.props.diameter / 2);

        for (let k = first; k <= last; k++) {
            for (let i = first; i <= last; i++) {
                for (let j = first; j <= last; j++) {
                    let offset = this.props.diameter % 2 === 0 ? 0.5 : 0;
                    let d = Math.sqrt((i + offset) * (i + offset) + (j + offset) * (j + offset) + (k + offset) * (k + offset));
                    if (d < Math.floor(this.props.diameter / 2) + 0.5) {
                        let I = ijk.i + i;
                        let J = ijk.j + j;
                        let K = ijk.k + k;
                        let doSet = true;
