namespace KullaGrid {
    ...
    export class ChunckMeshBuilder {
        ...
        private _GetNormal(x: number, y: number, z: number): BABYLON.Vector3 {
            //z = Math.floor(z / 2);
            //if (y % 2 === 1) {
            //    x = x / 2;
            //    z = z / 2;
            ...
            if (!this._Normals[y][x][z]) {
                this._Normals[y][x][z] = BABYLON.Vector3.Zero();
            }
            
            return this._Normals[y][x][z];
        }
        
        public BuildMesh(chunck: Chunck, sides: number, analyticOccurence?: IChunckAnalyticBuildOccurence): BABYLON.VertexData {
            this._Vertices = [];
            this._Normals = [];
                ...
                for (let k = firstNonEmptyReferenceK; k < lastNonEmptyReferenceK; k++) {
                    for (let j = - m; j < chunck.chunckLengthIJ + m; j++) {
                        for (let i = - m; i < chunck.chunckLengthIJ + m; i++) {
                            ...
                            if (isFinite(reference) && reference != 0 && reference != 0b11111111) {
        ...
        public BuildGridMesh(chunck: Chunck, ijk: Nabu.IJK, radius: number, color: BABYLON.Color3, alphaMax: number = 1): BABYLON.VertexData {
